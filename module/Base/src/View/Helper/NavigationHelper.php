...
class NavigationHelper extends AbstractHelper{
    ...
    public function nav_user(){
        ...
        $minhaConta=$this->view->Html('a')->setAttributes(['href'=>$this->view->url('auth/default',['controller'=>'auth','action'=>'update-profile'])])->setText("MINHA CONTA");
        $liMinhaConta=$this->view->Html('li')->setText($minhaConta);
        $dropdown_menu->setText($liMinhaConta);

        $updatePass=$this->view->Html('a')->setAttributes(['href'=>$this->view->url('auth/default',['controller'=>'auth','action'=>'update-password'])])->setText("ALTERAR SENHA");
